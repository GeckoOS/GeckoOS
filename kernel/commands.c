...
static void cmd_help(uint8_t color) {
    ...
    printc("\n--- Filesystem ---\n", VGA_COLOR_LIGHT_CYAN);
    printc("fsmount     - Mount the FAT32 filesystem\n", color);
    printc("ls          - List root directory\n", color);
    printc("cat         - Print a file's contents\n", color);
    printc("fsinfo      - Filesystem volume info\n", color);
    printc("touch       - Create a file (prompts)\n", color);
    printc("rm          - Delete a file\n", color);
    printc("cp          - Copy a file\n", color);
    printc("mv          - Move/rename a file\n", color);
    printc("mkdir       - Create a directory\n", color);
    printc("echo        - Print text to screen\n", color);
    printc("write       - Append text to a file\n", color);
    printc("\n", color);
}
