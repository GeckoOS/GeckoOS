 ...
 *
 * Multiboot2 specification constants and tag structures.
 * Reference: https://www.gnu.org/software/grub/manual/multiboot2/multiboot.html
 *
 * GeckoOS uses these to parse the boot information structure passed by GRUB
 * in the EBX register at kernel entry.
 */
...
/**
 * GRUB writes this value into EAX before jumping to the kernel entry point.
 * The kernel checks EAX == MULTIBOOT2_BOOTLOADER_MAGIC to confirm it was
 * launched by a Multiboot2-compliant bootloader.
 */
...
/* ── Boot information tag types (in the MBI passed by GRUB) ─────────────── */
...
/**
 * Fixed header at the start of every Multiboot2 boot information structure.
 * GRUB passes a pointer to this in EBX.
 */
