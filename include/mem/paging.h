...
#define PT_ENTRIES       512        /* entries per any level table */
...
typedef enum {
    ...
    PTE_ACCESSED       = (1ULL << 5),
    PTE_DIRTY          = (1ULL << 6),
    PTE_HUGE           = (1ULL << 7),   /* 2 MB page at PD level */
    PTE_GLOBAL         = (1ULL << 8),
    PTE_NX             = (1ULL << 63),  /* No-Execute (requires EFER.NXE) */
    PTE_ADDR_MASK      = 0x000FFFFFFFFFF000ULL
} pte_flags_t;

typedef uint64_t pte_t;

/* One page table / page directory / PDPT / PML4 — all the same shape */
typedef struct {
